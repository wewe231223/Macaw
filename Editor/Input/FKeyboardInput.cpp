...
#ifdef OBJ_VIEWER
        if (mWorldCommandSender.has_value() && (NavigationInput.mForwardAxis != 0.0f || NavigationInput.mRightAxis != 0.0f)) {
            mWorldCommandSender->TryEmplace<FKeyboardCameraMoveRequestMessage>(NavigationInput.mForwardAxis, NavigationInput.mRightAxis, NavigationInput.mDeltaTime);
        }
#endif
    }
